...
export default function Map({campsites=[],route=null,stops=[],fuelStations=[],onSelect}:any){
 ...
 const points=[...campsites.map((c:any)=>[c.longitude,c.latitude]),...stops.map((s:any)=>[s.longitude,s.latitude]),...fuelStations.map((s:any)=>s.geometry?.coordinates).filter(Boolean)];if(points.length&&(route?.geometry||stops.length)){const b=new mapboxgl.LngLatBounds();points.forEach((p:any)=>b.extend(p as [number,number]));map.fitBounds(b,{padding:60,maxZoom:11});}
